...
- **Dashboard:** at-risk jobs list under delivery pipeline
- **Letters:** copy text + print; draft/sent counts
- **Portal:** soft white cards, progress bars on work, mint upload CTA
- **Workload:** relative load bars + overdue board link
- **Sidebar:** red overdue badge on Jobs nav
- **apiClient.patch** added (fixes TS for jobs/letters)
...
Cutover still blocked until happy. Optional next: Playwright e2e, automation builder, SMS.
```
...
- **Verify:** signed ping → 200 `{received:true}` on connect + platform webhooks
...
Optional: commit+deploy account.updated harden on engage-from-capstone master.
```
...
- AF: still mock-only; clone accountflow-practice isolated
...
- Automations: 5 UK packs (VAT/SA/Sales/Delivery/MTD) install into local builder
- Portal: soft metal header + KPI strip + job cards
- Jobs board columns use MetalProgress
- Stack: :3101 / :5273
...
- Automation rules server-side: tenant.settings.automationRules; PUT/GET/run dry|live
- Practice letters: PATCH bodyHtml + block designer (header/body/services/fees/clauses/signoff)
...
- ClaraBoardPriorities on Jobs board
- Job COMPLETE → renewal window activity nudge + UI banner
...
4. E2E: practice-w3-surfaces.spec.ts **6/6 passed** chromium
...
- Partner programme metal refresh
- practice-e2e.yml (path-filtered) + playwright test_filter input
