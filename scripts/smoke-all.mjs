...
async function run(script) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [path.join(__dirname, script), ...process.argv.slice(2)], {
      stdio: 'inherit',
      env: process.env,
    });
...
console.log('Running combined smoke (single login per script — run this instead of multiple scripts)\n');
