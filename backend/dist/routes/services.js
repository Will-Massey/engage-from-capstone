 ...
 */
router.post('/', authenticate, authorize('ADMIN', 'PARTNER', 'MANAGER'), asyncHandler(async (req, res) => {
 ...
 */
router.put('/:id', authenticate, authorize('ADMIN', 'PARTNER', 'MANAGER'), asyncHandler(async (req, res) => {
    const { id } = req.params;
 ...
 */
router.post('/:id/duplicate', authenticate, authorize('ADMIN', 'PARTNER', 'MANAGER', 'SENIOR'), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const existingService = await prisma.serviceTemplate.findFirst({
        where: {
            id,
    ...
    await Promise.all(pricingRules.map((rule) => prisma.pricingRule.create({
        data: {
            ...rule,
            id: undefined,
 ...
 */
router.post('/:id/pricing-rules', authenticate, authorize('ADMIN', 'PARTNER', 'MANAGER'), asyncHandler(async (req, res) => {
    const { id } = req.params;
