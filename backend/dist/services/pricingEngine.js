...
export class PricingEngine {
     ...
     */
    async calculateProposalPricing(services, clientData, globalDiscount) {
        const serviceCalculations = await Promise.all(services.map(async (svc) => {
     ...
     */
    calculateComplexityMultiplier(factors, clientData) {
        let multiplier = 1;
        // Apply transaction volume factor
        if (clientData.transactionVolume) {
            if (clientData.transactionVolume > 500)
                multiplier *= 1.3;
            else if (clientData.transactionVolume > 200)
                multiplier *= 1.15;
            else if (clientData.transactionVolume > 50)
                multiplier *= 1.05;
        }
        ...
        if (clientData.employeeCount) {
            if (clientData.employeeCount > 50)
                multiplier *= 1.25;
            else if (clientData.employeeCount > 20)
                multiplier *= 1.15;
            else if (clientData.employeeCount > 5)
                multiplier *= 1.05;
        }
        return multiplier;
    }
     ...
     * Apply pricing rules
     */
    async applyPricingRules(basePrice, rules, clientData) {
     ...
     */
    evaluateCondition(condition, clientData) {
     ...
     * Calculate annual value based on frequency
     */
    calculateAnnualValue(price, frequency) {
        switch (frequency) {
            case 'MONTHLY':
                return price * 12;
            case 'QUARTERLY':
