...
export async function getBenchmarkPricing(
  ...
  if (optedIn) {
    const optInTenants = await prisma.tenant.findMany({
      where: { isActive: true },
      select: { id: true, settings: true },
      take: 200,
    });
    tenantIds = optInTenants
      .filter((t) => parseBenchmarkOptIn(t.settings))
      .map((t) => t.id);
    if (!tenantIds.includes(tenantId)) tenantIds.push(tenantId);
  }

  let bands = await aggregateBenchmarks(tenantIds, serviceNames);
  if (!bands.length) {
    bands = UK_STUB_BENCHMARKS;
    if (serviceNames?.length) {
      const lower = serviceNames.map((s) => s.toLowerCase());
  ...
  return {
    ...
    disclaimer: hasLive
      ? optedIn
        ? 'Figures combine anonymised accepted fees from opt-in UK practices plus your own data.'
        : 'Figures are based on your practice accepted proposals (annualised).'
      : optedIn
        ? 'Insufficient accepted proposal data yet — showing indicative UK practice norms.'
        : 'Enable benchmark pricing in Settings to compare with anonymised UK practice data.',
  };
}