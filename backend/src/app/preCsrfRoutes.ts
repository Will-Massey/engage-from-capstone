...
export function mountPreCsrfRoutes(app: express.Express): void {
  ...
  app.get('/api/seed-services-public', async (req, res) => {
    ...
    if (!secureCompare(secret, expected)) {
      res
        .status(403)
        .json({ success: false, error: { code: 'FORBIDDEN', message: 'Invalid key' } });
      return;
    }
    ...
    END $$;`);
      ...
      res
        .status(500)
        .json({ success: false, error: { code: 'SEED_ERROR', message: error.message } });
    }
  });
}
