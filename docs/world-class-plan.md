...
| 1.6 | 🔶 `any` burn-down (289): type api call sites module-by-module with `ApiResponse<T>` — auth module typed (`frontend/src/types/auth.ts`)                    | Typed seams catch integration drift at compile time                                    | tsc strict; count tracked in CI log                                                                                                                | M (rolling) | NEXT  |
