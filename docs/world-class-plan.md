...
| 1.6 | 🔶 `any` burn-down (289): auth … payout settings + Clara AI endpoints typed (`types/auth.ts` … `types/ai.ts`)                                              | Typed seams catch integration drift at compile time                                    | tsc strict; count tracked in CI log                                                                                                                | M (rolling) | NEXT  |
