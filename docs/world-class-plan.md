...
| 1.6 | 🔶 `any` burn-down (289): auth … engagement library + Companies House API typed (`types/auth.ts` … `types/companiesHouse.ts`)                              | Typed seams catch integration drift at compile time                                    | tsc strict; count tracked in CI log                                                                                                                | M (rolling) | NEXT  |
