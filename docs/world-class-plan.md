...
| 1.6 | 🔶 `any` burn-down (289): auth + proposals + clients + services API typed (`types/auth.ts`, `types/proposals.ts`, `types/clients.ts`, `types/services.ts`) | Typed seams catch integration drift at compile time                                    | tsc strict; count tracked in CI log                                                                                                                | M (rolling) | NEXT  |
