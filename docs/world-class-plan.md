...
| 1.6 | 🔶 `any` burn-down (289): auth … proposal workflow + payments/billing API typed (`types/auth.ts` … `types/payment.ts`)                                     | Typed seams catch integration drift at compile time                                    | tsc strict; count tracked in CI log                                                                                                                | M (rolling) | NEXT  |
