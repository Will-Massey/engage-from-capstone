...
| 1.6 | 🔶 `any` burn-down (289): auth + proposals + clients + services + tenants + pricing AI API typed (`types/auth.ts` … `types/pricing.ts`)                    | Typed seams catch integration drift at compile time                                    | tsc strict; count tracked in CI log                                                                                                                | M (rolling) | NEXT  |
