...
| 1.2 | 🔶 **Frontend test foundation**: vitest for auth store, builder actions, public signing helpers (31 tests)                                                 | Every UI regression currently rides on 8 smoke specs                                   | Vitest + testing-library for: auth store, api interceptors (CSRF/refresh), ProposalBuilderContext actions (the 97-field core), public signing page | M           | NOW   |
...
| 1.6 | 🔶 `any` burn-down (289): auth + proposals + clients API typed (`types/auth.ts`, `types/proposals.ts`, `types/clients.ts`)                                 | Typed seams catch integration drift at compile time                                    | tsc strict; count tracked in CI log                                                                                                                | M (rolling) | NEXT  |
