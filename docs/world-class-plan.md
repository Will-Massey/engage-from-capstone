...
| 1.6 | 🔶 `any` burn-down (289): auth … pricing + touchpoints/automation API typed (`types/auth.ts` … `types/touchpoints.ts`, `types/automation.ts`)              | Typed seams catch integration drift at compile time                                    | tsc strict; count tracked in CI log                                                                                                                | M (rolling) | NEXT  |
