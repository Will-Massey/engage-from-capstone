...
| 1.6 | 🔶 `any` burn-down (289): auth … templates + engagement library API typed (`types/auth.ts` … `types/engagementLibrary.ts`)                                 | Typed seams catch integration drift at compile time                                    | tsc strict; count tracked in CI log                                                                                                                | M (rolling) | NEXT  |
