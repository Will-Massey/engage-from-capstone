...
| 1.2 | 🔶 **Frontend test foundation**: vitest for auth store, draft helpers, ProposalBuilderContext save/service actions (20 tests)                              | Every UI regression currently rides on 8 smoke specs                                   | Vitest + testing-library for: auth store, api interceptors (CSRF/refresh), ProposalBuilderContext actions (the 97-field core), public signing page | M           | NOW   |
...
| 1.6 | 🔶 `any` burn-down (289): auth + proposals API typed (`frontend/src/types/auth.ts`, `types/proposals.ts`)                                                  | Typed seams catch integration drift at compile time                                    | tsc strict; count tracked in CI log                                                                                                                | M (rolling) | NEXT  |
