...
const OnboardingTour = ({ isOpen, onClose }: OnboardingTourProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4">
      <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm" onClick={dismiss} aria-hidden />
      <div
      ...
      >
        ...
        <h2 id="onboarding-tour-title" className="mt-2 text-xl font-semibold text-slate-900 dark:text-white">
          {current.title}
        </h2>
        <p className="mt-2 text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{current.body}</p>
        <div className="mt-6 flex flex-wrap gap-2 justify-between">
          <button type="button" onClick={dismiss} className="text-sm text-slate-500 hover:text-slate-700">
            Skip tour
          </button>
          <div className="flex gap-2">
            ...
            {isLast ? (
              ...
              <button type="button" onClick={() => setStep((s) => s + 1)} className="btn-primary text-sm">
                Next
              </button>
            )}
          </div>
